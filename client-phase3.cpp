...
//yet to make hash unhash functions

string calc_MD5(string content){
...
int main(int argc, char *argv[]){
    ...
    if( pDIR=opendir(argv[1])){
        while(entry = readdir(pDIR)){
            if(strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
                ...
                filehash[entry->d_name]=calc_MD5(entry->d_name);
            }
        closedir(pDIR);
    }
    ...
    for (auto *it=filemap.begin(); it!=filemap.end();it++){
        ...
        else cout<<"Found "<<it->first<<" at "<<it->second<<" with MD5 "<<filehash[s]<<" at depth 1"<<endl;
    }
}
